...
- Local Android production builds use timestamped directories under `$HOME/.cache/eas-local-builds` because `/tmp` may be tmpfs-backed and too small for native C++ builds. EAS local build working directories must be empty at startup.
